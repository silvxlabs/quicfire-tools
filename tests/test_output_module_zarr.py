...
class TestSimulationOutputs:
    ...
    def test_zarr_single_output(self, single_output_name="fuels-dens"):
        ...
        """
        self.sut.to_zarr(
            OUTPUT_PATH / "zarr_outputs", outputs=single_output_name,
            over_write=True
        )
    ...
    def test_zarr_multiple_outputs(self, multiple_output_names = ["mburnt_integ", "fuels-dens"]):
        """
        Run a test to ensure that the zarr output contains multiple outputs
        """
        self.sut.to_zarr(
            OUTPUT_PATH / "zarr_outputs", outputs=multiple_output_names,
            over_write=True
        )
        assert self.sut.zarr_folder_path.exists()
        for output_name in self.sut.list_available_outputs():
